...
namespace HR
{
    ...
    ";
        ...
        static void GetMaximumSalaryAndID(SqlConnection connection, int depID, out decimal maxSalary, out int maxID)
        {
            ...
            SELECT TOP 1 s.basicSalary, e.employeeID FROM hrdbEmployee e
            JOIN hrdbSalary s ON e.employeeID = s.employeeID WHERE e.empDepID = @depID ORDER BY s.basicSalary DESC";
